...
import { CATEGORIES, HIGH_PRIORITY_CATEGORIES } from '../constants/categories';
...
/**
 * MÓDULO 1: Monitor de Pulso - Sistema de Enjambre de Alertas
 * Detecta 3+ reportes de alta prioridad en un radio de 200m en menos de 5 minutos
 */
export async function detectCriticalAlerts() {
  const now = Date.now();
  const fiveMinutesAgo = new Date(now - 5 * 60 * 1000).toISOString();
  
  // Obtener todos los reportes de alta prioridad de los últimos 5 minutos
  ...
  for (let i = 0; i < highPriorityReports.length; i++) {
    ...
    // Si hay 3 o más reportes, crear alerta crítica
    if (cluster.length >= 3) {
...
/**
 * MÓDULO 2: Planificador Táctico - Generador de Directivas de Patrullaje
 * Analiza reportes de los últimos 7 días para un rango de horas específico
 */
export async function generatePatrolDirectives(shiftStart, shiftEnd) {
  ...
  // Agrupar por zonas (usando una cuadrícula aproximada)
  const zones = {};
  ...
  // Calcular centro y puntaje de riesgo para cada zona
  const directives = Object.entries(zones)
    .map(([key, zone]) => {
      ...
      // Determinar amenazas principales
    ...
    .filter(d => d.reportCount >= 3) // Solo zonas con al menos 3 reportes
...
/**
 * MÓDULO 3: Gestor de Riesgos - Matriz Causa-Efecto
 * Cruza datos de riesgos (postes sin luz, parques abandonados) con incidentes
 */
export async function analyzeRiskCorrelations() {
  const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 3600 * 1000).toISOString();
  const allReports = await fetchReports({ sinceISO: thirtyDaysAgo });

  // Separar reportes de riesgo e incidentes
  ...
  riskReports.forEach(riskReport => {
    // Contar incidentes a menos de 100 metros
